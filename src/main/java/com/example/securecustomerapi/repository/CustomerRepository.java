...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    List<Customer> findAllByStatus(@Param("status") CustomerStatus status);
    ...
    @Query("SELECT c FROM Customer c WHERE " +
           ...
           "(:status IS NULL OR LOWER(c.status) LIKE LOWER(CONCAT('%', :status, '%')))")
    List<Customer> advancedsearchCustomers(@Param("name") String name, @Param("email") String email, @Param("status") String status );
}